...
async function sendAcceptanceEmailToAgent(quoteId: string, quote: any, user: any) {
  const emailContent = `
    <h1>¡Oferta Aceptada!</h1>
    <p>El cliente ${user.name} (${user.email}) ha aceptado la siguiente oferta:</p>
    ...
    <hr>
    <p>Por favor, contacta al cliente para finalizar los detalles del viaje.</p>
  `;
  ...
  try {
    await resend.emails.send({
      ...
      subject: `Oferta aceptada por ${user.name}`,
      html: emailContent,
    });
  } catch (error) {
